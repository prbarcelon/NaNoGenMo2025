...
public class Cfg
{
    ...
    public void LoadVocabulary()
    {
                     ...
                     {"adjectives", "nouns", "verbs", "adverbs"})
        {
            ...
            var entries =
                JsonSerializer.Deserialize<List<VocabEntry>>(json, options);

            if (entries != null) _vocabulary[category] = entries;
        }
    ...
    private bool MatchesTags(
        TagSet entryTags, string? alignment, string? element, string? mastery,
        string? skillType
    )
    {
    ...
    /// </summary>
    private bool HasBannedTags(
        TagSet entryTags, HashSet<string> accumulatedTags,
    ...
    private VocabEntry GenerateNumberEntry()
    {
        ...
        var formats = new[]
        {
            ...
            number <= 20
                ? $"{GetNumberWord(number)}s"
                : null // "Twos", "Nines" (for 1-20)
        };
    ...
    )
    {
        var filtered = _vocabulary[category]
            .Where(entry => MatchesTags(entry.Tags, alignment, element, mastery,
                skillType))
            .Where(entry =>
                !HasBannedTags(entry.Tags, accumulatedTags, bannedTags))
            .ToList();
    ...
    private TagSet AccumulateTags(params VocabEntry[] entries)
    {
        ...
        {
            foreach (var tag in entry.Tags.Alignment)
                accumulated.Alignment.Add(tag);
            foreach (var tag in entry.Tags.Element)
                accumulated.Element.Add(tag);
            foreach (var tag in entry.Tags.Mastery)
                accumulated.Mastery.Add(tag);
            foreach (var tag in entry.Tags.SkillType)
                accumulated.SkillType.Add(tag);
        }
    ...
    )
    {
        ...
        var vocabLists = parts
            .Select(part => GetFilteredVocab(partMap[part], alignment, element,
                mastery, skillType, accumulatedTags, bannedTags))
            .ToList();
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            if (index >= 0)
                text = text[..index] + vocabEntries[i].Word.Titleize() +
    ...
    private bool HasStemConflicts(params List<VocabEntry>[] levelEntries)
    {
        ...
        for (var i = 0; i < levelStems.Count; i++)
        for (var j = i + 1; j < levelStems.Count; j++)
    ...
    )
    {
        ...
        var sampled = 0;
        // Allow many attempts to find unique combinations
        var maxAttempts = maxCombinations * 50;
        ...
        Parallel.For(0, maxAttempts, (_, state) =>
        {
            ...
            lock (_random)
            {
                ...
                var artsRule =
                    _grammarRules["Arts"][
                        _random.Next(_grammarRules["Arts"].Count)];
                arts = GenerateRandomExpansion(artsRule, alignment, element,
                    mastery, skillType, accumulatedTags, _bannedTags);
                if (arts == null)
                    return;
                ...
                var styleRule =
                    _grammarRules["Style"][
                        _random.Next(_grammarRules["Style"].Count)];
                style = GenerateRandomExpansion(styleRule, alignment, element,
                    mastery, skillType, accumulatedTags, _bannedTags);
                if (style == null)
                    return;
                ...
                var formRule =
                    _grammarRules["Form"][
                        _random.Next(_grammarRules["Form"].Count)];
                form = GenerateRandomExpansion(formRule, alignment, element,
                    mastery, skillType, accumulatedTags, _bannedTags);
                if (form == null)
                    return;
                ...
                var techRule =
                    _grammarRules["Technique"][
                        _random.Next(_grammarRules["Technique"].Count)];
                tech = GenerateRandomExpansion(techRule, alignment, element,
                    mastery, skillType, accumulatedTags, _bannedTags);
                if (tech == null)
                    return;
            }
            ...
            if (HasStemConflicts(arts.Entries, style.Entries, form.Entries,
                    tech.Entries))
                return;
            ...
            var fullName =
                $"{arts.Text}, {style.Text}, {form.Text}: {tech.Text}";
            ...
            lock (lockObj)
            {
                ...
                if (sampled % 1000 == 0)
                    Console.WriteLine(
                        $"  Progress: {sampled:N0} / {maxCombinations:N0}");
            }
        });

        Console.WriteLine(
            $"Generated {results.Count:N0} unique valid combinations");
        return results;
    }
    ...
    public void SaveOutput(List<Technique> techniques, string outputPath)
    {
        ...
        Console.WriteLine(
            $"Saved {techniques.Count:N0} techniques to {outputPath}");
    }
}
...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        var vocabDir = "data"; // Default to ./data
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            if (!args[i].StartsWith("--") &&
                int.TryParse(args[i], out var positionalMax))
            {
        ...
        {
            Console.WriteLine(
                "Error: Number of combinations is required (provide a number as argument)");
            PrintHelp();
            return;
        }
        ...
        try
        {
            ...
            var techniques = generator.GenerateAll(maxCombinations.Value,
                alignment, element, mastery, skillType);
            ...
            for (var i = 0; i < Math.Min(previewCount, techniques.Count); i++)
            {
                ...
                Console.WriteLine(
                    $"   (Alignment: {alignmentStr}, Element: {elementStr}, Mastery: {masteryStr}, Skill: {skillTypeStr})");
            }
